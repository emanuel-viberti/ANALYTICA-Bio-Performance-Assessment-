...
with col_res:
    ...
    st.metric("Brazo Ajustado", f"{brazo_final:.1f} cm")
    if brazo_final >= punto_corte:
        st.success("Masa Adecuada")
    else:
        st.warning("Bajo el óptimo")

with col_graf:
    fig = go.Figure(go.Bar(
        x=[brazo_final, punto_corte],
        y=['Tu Medida', 'Objetivo Mín.'],
        orientation='h',
        marker_color=['#00FFC8', '#3E444E']
    ))
    fig.update_layout(
...
with st.expander("🔬 ¿Por qué medimos el brazo? (Base Científica)"):
    st.write("""
        ...
        **Puntos clave del método:**
        ...
        3. **Referencia Científica:** Los datos se contrastan con una base de datos de más de 18.000 sujetos (NHANES 1999–2006) para determinar rangos de funcionalidad y salud metabólica.
    """)
    st.caption("Fuente: Arm circumference as a marker of muscle mass: cutoff values from NHANES (AJCN, 2025).")
...
# 5. RESULTADOS Y GRÁFICO COMPARATIVO
st.write("---")
col_res, col_graf = st.columns([1, 2])
...
with st.expander("🔬 ¿Por qué medimos el brazo? (Base Científica)"):
    st.write("""
        ...
        **Puntos clave del método:**
        ...
        2. **Ajuste por Grasa:** Aplicamos una corrección matemática según tu IMC para filtrar el tejido adiposo.
        3. **Referencia:** Comparativa con una base de datos de 18.000+ personas para determinar rangos de salud y rendimiento.
    """)
    st.caption("Fuente: AJCN, 2025.")
